...
import generateServiceTicketOpenAdminPDF from "../OpenticketAdmin/pdfGeneratoropenadmin";
...
const CloseticketAdmin = () => {
  ...
  const [filterCriteria, setFilterCriteria] = useState({
    ...
    time: "",
    status: "",
    eta: "", // Add ETA field to filter criteria
  });
    ...
    const fetchTickets = async () => {
      ...
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_API_URL}/tickets/closed`
        );

        // Process the tickets to include ETA information
        const processedTickets = response.data.map((ticket) => ({
          ...ticket,
          eta: ticket.eta?.totalDays || 0,
        }));
  ...
  useEffect(() => {
    ...
    selectedCategories,
    dateRange,
    // No need to add individual etaMin/etaMax as they're part of filterCriteria
  ]);
  ...
  useEffect(() => {
    if (tickets.length > 0) {
      ...
      const categories = [
        ...new Set(
          tickets.map((ticket) => ticket.issueCategory).filter(Boolean)
        ),
      ];
                      ...
                      .map((ticket) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <td
                              className={`whitespace-nowrap px-2 py-2 font-poppins text-neutral-900 text-center ${getEtaBackgroundColor(
                                ticket.eta
                              )}`}
                            >
                              {ticket.eta || "0"}
